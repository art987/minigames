...
// 获取第31-45个品牌
const thirdBatchBrands = brandsWithoutAdvantages.slice(30, 45);

console.log(`找到没有核心优势的品牌总数: ${brandsWithoutAdvantages.length}`);
console.log(`第三批处理的品牌（第31-45个）: ${thirdBatchBrands.length}个`);

// 保存到JSON文件
const outputPath = path.join(__dirname, 'thirdBatchBrandsToAddAdvantages.json');
fs.writeFileSync(outputPath, JSON.stringify(thirdBatchBrands, null, 2), 'utf8');

console.log('第三批品牌数据已保存到thirdBatchBrandsToAddAdvantages.json');