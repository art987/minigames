...
function main() {
  ...
  // 提取第121-135个品牌（索引从0开始，所以是120-134）
  const startIndex = 120;
  const endIndex = 134;
  const ninthBatchBrands = brandsWithoutAdvantages.slice(startIndex, endIndex + 1);
  
  console.log(`成功提取第 ${startIndex + 1}-${endIndex + 1} 个品牌，共 ${ninthBatchBrands.length} 个`);
  
  // 保存到文件
  const outputPath = path.join(__dirname, 'ninthBatchBrandsToAddAdvantages.json');
  fs.writeFileSync(outputPath, JSON.stringify(ninthBatchBrands, null, 2), 'utf8');
