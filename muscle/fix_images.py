...
# 简单的替换方法：找到所有肌肉数据行并添加images字段
lines = content.split('\n')
new_lines = []

for line in lines:
    # 检查是否是肌肉数据行
    if '"name":"' in line and '"part":"' in line and '"group":"' in line:
        # 提取肌肉名称
        ...
        if name_match:
            ...
            # 在action字段后添加images字段
            if '"action":"' in line:
                # 找到action字段的结束位置
                action_end = line.find('"', line.find('"action":"') + 10)
                if action_end != -1:
                    # 在action字段后添加images字段
                    new_line = line[:action_end+1] + ',"images":["medias/' + pinyin_name + '.jpg"]' + line[action_end+1:]
