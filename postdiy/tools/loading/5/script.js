 ...
 * linear
 *
 * @param {number} t time: 鐝惧湪鏅傚埢 (0 ~ duration)
 * @param {number} b begin: 闁嬪浣嶇疆
 * @param {number} c change: 闁嬪浣嶇疆銇嬨倝绲備簡浣嶇疆銇俱仹銇鍖栭噺 (finish - begin)
 * @param {number} d duration: 鍏ㄤ綋鏅傞枔
 * @returns {number} 鐝惧湪浣嶇疆
 */
 ...
 * 銈€儖銉°兗銈枫儳銉抽枹鏁�
 *
 * @param {AnimationCallback} fn 銈€儖銉°兗銈枫儳銉炽儠銉兗銉犳瘞銇疅琛屻仚銈嬨偝銉笺儷銉愩儍銈�
 * @param {Object} [options={}] 銈儣銈枫儳銉�
 * @param {number} [options.begin=0] 闁嬪浣嶇疆
 * @param {number} [options.finish=1] 绲備簡浣嶇疆
 * @param {number} [options.duration=500] 鍏ㄤ綋鏅傞枔
 ...
 * @param {number} position 鐝惧湪浣嶇疆
 * @param {number} time 鐝惧湪鏅傚埢 (0 ~ duration)
 */
...
function $vFOr$export$getTextCoordinate(option) {
  ...
  document.body.appendChild(canvas); // letter-spacing 銈掓湁鍔广伀銇欍倠銇熴倎銇繀瑕�
