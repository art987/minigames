...
// 使用正则表达式替换重复的logo字段，保留第一个
content = content.replace(/(\{name: "[^\"]+",\s*logo: "[^\"]+")\s*,\s*logo: "[^\"]+"/g, '$1');
...
for (const [chineseName, englishName] of Object.entries(brandNameMap)) {
    const oldPathPattern = new RegExp(`name: "${chineseName}",\s*logo: "images/skincare/logos/${chineseName}\.png"`, 'g');
    const newPath = `name: "${chineseName}", logo: "logos/skincare/${englishName}.png"`;
